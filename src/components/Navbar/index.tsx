...
const NavLinkWrapper: React.FC<any> = ({ children, ...props }) => {
  ...
  // Native fallback: use react-native primitives when running on device
  try {
    ...
    // If react-native isn't available, render children as-is
    return <>{children}</>;
  }
};
    ...
    <div style={styles.navbar}>
      ...
      <nav style={styles.itemsContainer} aria-label="Main navigation">
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
          ...
          <NavLinkWrapper to="/dashboard" style={({ isActive }: any) => ({ ...styles.item, opacity: isActive ? 1 : 0.85 })}>
            Dashboard
          </NavLinkWrapper>
        </div>
        <NavLinkWrapper to="/apps" style={({ isActive }: any) => ({ ...styles.item, opacity: isActive ? 1 : 0.85 })}>
          Apps
        </NavLinkWrapper>
        <NavLinkWrapper to="/explore" style={({ isActive }: any) => ({ ...styles.item, opacity: isActive ? 1 : 0.85 })}>
          Explore
        </NavLinkWrapper>
      </nav>
        ...
        <div style={styles.loginWrapper}>
          ...
          {isMenuOpen && (
            <div style={styles.dropdownMenu}>
              <NavLinkWrapper
                to="/login"
                style={({ isActive }: any) => ({ ...styles.dropdownItem, opacity: isActive ? 1 : 0.85 })}
